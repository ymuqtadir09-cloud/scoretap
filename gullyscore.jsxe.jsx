...
const C = {
  ...
  four: "#3B82F6", six: "#A855F7", wide: "#F59E0B", noball: "#EF4444", wicket: "#EF4444",
};

const injectFonts = () => {
  if (document.getElementById("gs-fonts")) return;
  const s = document.createElement("style");
  s.id = "gs-fonts";
  s.textContent = `
    @import url('https://fonts.googleapis.com/css2?family=Space+Grotesk:wght@400;500;600;700;800&family=Inter:wght@400;500;600&display=swap');
    * { box-sizing: border-box; margin: 0; padding: 0; }
    body { background: #0A0E0A; font-family: 'Inter', sans-serif; color: #F0FDF4; overflow-x: hidden; }
    ::-webkit-scrollbar { width: 4px; } ::-webkit-scrollbar-track { background: #0A0E0A; } ::-webkit-scrollbar-thumb { background: #15803D; border-radius: 2px; }
    @keyframes pulse { 0%,100%{opacity:1;transform:scale(1)} 50%{opacity:.6;transform:scale(.9)} }
    @keyframes slideUp { from{opacity:0;transform:translateY(16px)} to{opacity:1;transform:translateY(0)} }
    @keyframes pop { 0%{transform:scale(1)} 50%{transform:scale(1.18)} 100%{transform:scale(1)} }
    @keyframes glow { 0%,100%{box-shadow:0 0 0 0 rgba(34,197,94,0.18)} 50%{box-shadow:0 0 18px 4px rgba(34,197,94,0.18)} }
  `;
...
// ── Storage helpers (no dummy data ever)
function loadMatches() {
  try { const s = localStorage.getItem("gs_matches"); return s ? JSON.parse(s) : []; } catch { return []; }
}
function saveMatches(matches) {
  try { localStorage.setItem("gs_matches", JSON.stringify(matches)); } catch {}
}
...
function HomeScreen({ onStart, matches }) {
  return (
    <div style={{ padding: "20px 16px 100px", animation: "slideUp .3s" }}>
      ...
      {/* Recent matches — always shown, empty state when none */}
      <div style={{ marginTop: 28 }}>
        <div style={{ fontFamily: "'Space Grotesk'", fontWeight: 700, fontSize: 15, color: C.textDim, marginBottom: 12, letterSpacing: .5 }}>
          RECENT MATCHES
        </div>
        {matches.length === 0 ? (
            ...
            <Card key={i} style={{ marginBottom: 10 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                ...
                <div style={{ textAlign: "right" }}>
                  {m.winner ? <Pill color={C.green}>{m.winner} won</Pill> : <Pill color={C.amber}>In Progress</Pill>}
                </div>
              </div>
            </Card>
          ))
...
// ── Over display component — current on top, previous below in reverse
function OverDisplay({ overGroups }) {
    ...
    return (
      <div style={{ padding: "0 16px 12px" }}>
        <div style={{
          ...
          border: `1.5px solid ${C.green}55`, boxShadow: `0 0 10px 0 rgba(34,197,94,0.18)`,
        }}>
  ...
  return (
    <div style={{ padding: "0 16px 12px" }}>
      {/* Current over — always on top, highlighted when live */}
      <div style={{
        background: C.card, borderRadius: 12, padding: "10px 14px",
        ...
        <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
          ...
          {/* Empty slot placeholders for remaining balls in this over */}
      ...
      {/* Completed previous overs — newest first */}
      {overGroups.length > 1 && (
        <div>
          <div style={{ fontSize: 10, color: C.textMuted, fontWeight: 600, letterSpacing: .6, marginBottom: 6, paddingLeft: 2 }}>
            PREVIOUS OVERS
          </div>
          {[...overGroups.slice(0, currentIdx)].reverse().map((ov, ri) => {
            ...
            return (
              <div key={oi} style={{
                ...
                <div style={{ fontSize: 11, color: C.textMuted, fontWeight: 700, minWidth: 24, textAlign: "right" }}>
                  {overRuns}
                </div>
              </div>
            );
...
function ScoringScreen({ match, onMatchEnd, onBack }) {
  ...
  // Build over groups from ball array
  const overGroups = [];
  let cur = [], legals = 0;
  ...
  const addBall = (event) => {
    ...
    else if (event === "W") { triggerPop("OUT!", C.wicket); }
    ...
    if (chaseWon) {
      setShowMatchEnd({ winner: curTeam, margin: `${10 - newWickets} wickets`, score: `${newScore}/${newWickets}` });
      return;
    }
    if (allOut || oversUp) {
      if (innings === 1) {
        setFirstInningsScore(newScore);
        setShowInningsBreak({ score: newScore, wickets: newWickets, team: curTeam });
      } else {
        const lost = newScore < (firstInningsScore + 1);
        const otherTeam = curTeam === match.teamA ? match.teamB : match.teamA;
        setShowMatchEnd({
          winner: lost ? otherTeam : curTeam,
          margin: lost ? `${firstInningsScore + 1 - newScore} runs` : `${10 - newWickets} wickets`,
          score: `${newScore}/${newWickets}`,
          firstScore: `${firstInningsScore}`,
        });
      }
    }
  };
  ...
  const startInnings2 = () => {
    setInnings(2); setScore(0); setWickets(0); setBalls([]);
  ...
  return (
    <div style={{ padding: "0 0 120px", animation: "slideUp .3s" }}>
      ...
      {/* Over display — fixed vertical layout */}
  ...
  return (
    <div style={{ padding: "20px 16px 80px", animation: "slideUp .3s" }}>
      ...
      <Card style={{ marginBottom: 14 }}>
        <div style={{ fontFamily: "'Space Grotesk'", fontWeight: 700, marginBottom: 10 }}>🔗 Share Scorecard</div>
        <div style={{ background: C.surface, borderRadius: 10, padding: "10px 12px", fontFamily: "monospace", fontSize: 13, color: C.green, letterSpacing: 2, textAlign: "center", marginBottom: 10 }}>
          GS-{Math.random().toString(36).substring(2, 8).toUpperCase()}
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
          <Btn color={C.green} size="sm" style={{ padding: "10px 0" }}>Copy Link</Btn>
          <Btn color={C.blue} size="sm" style={{ padding: "10px 0" }}>QR Code</Btn>
        </div>
      </Card>
      <Btn onClick={onHome} size="lg" style={{ width: "100%", marginTop: 4 }}>← Back to Home</Btn>
    </div>
  );
}
